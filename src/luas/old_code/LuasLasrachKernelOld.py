...
class LuasLasrachKernel(Kernel):
    ...
    def decomp_no_stored_values(
        ...
        self.Sigma[1-self.cel_dim].cholesky_decomp(hp, non_cel_vec)

        # Generate transformed objects, doesn' actually do transformation yet
        K_tilde = self.Sigma[1-self.cel_dim].cholesky_transform(self.K[1-self.cel_dim])

        # Evaluates transformation and does eigendecomp
        stored_values["lam_K_tilde"], stored_values["Q_K_tilde"] = K_tilde.eigendecomp(hp, non_cel_vec)
    ...
    def logL(self, hp, x_l, x_t, R, stored_values):
        ...
        if self.Sigma[self.cel_dim].kf is None and type(self.K[1-self.cel_dim]) == Outer:
            ...
            L_inv_R, logdetK_block = general_cho_solve_vmap(self.K[self.cel_dim], self.Sigma[self.cel_dim], stored_values["lam_K_tilde"][-non_cel_rank:],
                                   R_prime[-non_cel_rank:, :], hp, cel_vec)

            
            # stored_values["cel_decomp"], logdetK_cel = decomp_vmap(self.K[self.cel_dim].kf, K_cel_diag,
            #                                                        self.Sigma[self.cel_dim].kf, S_cel_diag,
            #                                                        cel_vec, stored_values["lam_K_tilde"][-non_cel_rank:])
    
            # L_inv_R = calc_Linv_vec_vmap(*stored_values["cel_decomp"], R_prime[-non_cel_rank:, :])

            Dt_inv = 1/jnp.sqrt(S_cel_diag)
            diag_part = R_prime[:-non_cel_rank, :] * Dt_inv
            
            r_K_inv_r = (L_inv_R**2).sum() + (diag_part**2).sum()
            
            stored_values["logdetK"] += logdetK_block.sum() + (non_cel_vec.shape[-1] - non_cel_rank) * jnp.log(S_cel_diag).sum()
            ...
            # stored_values["cel_decomp"], logdetK_cel = decomp_vmap(self.K[self.cel_dim].kf, K_cel_diag,
            #                                                        self.Sigma[self.cel_dim].kf, S_cel_diag,
            #                                                        cel_vec, stored_values["lam_K_tilde"])
    
            # L_inv_R = calc_Linv_vec_vmap(*stored_values["cel_decomp"], R_prime)
            r_K_inv_r = (L_inv_R**2).sum()
            
            stored_values["logdetK"] += logdetK_block.sum()
            
            
        return - 0.5 * r_K_inv_r - 0.5 * stored_values["logdetK"]  - 0.5 * R.size * jnp.log(2*jnp.pi), stored_values
