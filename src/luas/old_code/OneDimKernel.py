...
__all__ = ["OneDimKernel"]

# Ensure we are using double precision floats as JAX uses single precision by default
jax.config.update("jax_enable_x64", True)


class OneDimKernel(Kernel):
    ...
    def kf(self, hp, x1, x2, **kwargs):

        K = self.Sigma.evaluate(hp, x1, x2, **kwargs)
    ...
    def decomp_fn(
        ...
        self.Sigma.cholesky_decomp(hp, x)
        stored_values["logdetK"] = self.Sigma.logdet
    ...
    def logL(
        ...
        stored_values = self.decomp_fn(hp, x, stored_values = stored_values)
        
        # Solves for L^-1 r
        alpha = self.Sigma.cho_solve(r, transpose = 0)
    ...
    def generate_noise(
        ...
        Z = np.random.normal(size = (N, size))
        
        self.Sigma.cholesky_decomp(hp, x)
        R_draw = self.Sigma.cho_mult(Z)
    ...
    def solve(
        ...
        alpha = self.Sigma.cho_solve(r, transpose = 0)
        K_inv_R = self.Sigma.cho_solve(alpha, transpose = 1)
    ...
    def K_mult_vec(
        ...
        **kwargs,
        ...
        Kr = self.Sigma.left_mult(r, hp, x, x, **kwargs)
