...
def general_cholesky(K: JAXArray, stored_values = {}) -> Tuple[JAXArray, JAXArray]:
...
def general_cholesky_stored(
    ...
    if stored_values:
        stored_values["L_cho"], stored_values["logdetK"] = jax.lax.cond(jnp.allclose(K, stored_values["K"],
                                                                        rtol = rtol, atol = atol),
                                                                        lambda K: (stored_values["L_cho"], stored_values["logdetK"]),
                                                                        general_cholesky, K)
    else:
        stored_values["L_cho"], stored_values["logdetK"] = general_cholesky(K)
    
    return stored_values["L_cho"], stored_values["logdetK"]
...
class GeneralKernel(Kernel):
    ...
    def __init__(
        ...
        if decomp_fn is None:
            ...
            self.K_decomp_fn = general_cholesky_stored
        else:
    ...
    def decomp_fn(
        ...
        stored_values["L_cho"], stored_values["logdetK"] = self.K_decomp_fn(K, stored_values = stored_values)
        stored_values["K"] = K
