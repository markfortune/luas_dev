...
class BlockKernel(CovType):
    ...
    def decompose(self, *X, **kwargs):
        ...
        #  raise Exception("Use SingleKronTermKernel instead!")

        block_kernels = (self.Sigma[self.block_dim],)
        ...
        for i in range(len(self.K_list)):
            block_kernels += (self.K_list[i][self.block_dim],)
        ...
        def matrix_inv_sqrt_calc(r, lam_arr, transpose):
            ...
            kf_block = lam_arr[0] * block_kernels[0]
            for i in range(1, len(block_kernels)):
                kf_block += lam_arr[i] * block_kernels[i]

            kf_block, stored_values = kf_block.decompose(X[self.block_dim])
        ...
        def logL_calc(r, lam_arr):
            ...
            kf_block, _ = kf_block.decompose(X[self.block_dim])
            return kf_block.logL(r)

        self.matrix_inv_sqrt_calc_vmap = jax.vmap(matrix_inv_sqrt_calc, in_axes = (1-self.block_dim, 0, None), out_axes = 1-self.block_dim)
        self.matrix_sqrt_calc_vmap = jax.vmap(matrix_sqrt_calc, in_axes = (1-self.block_dim, 0, None), out_axes = 1-self.block_dim)
        self.logL_vmap = jax.vmap(logL_calc, in_axes = (1-self.block_dim, 0))
        
        R_prime, logdet_arr = self.matrix_sqrt_calc_vmap(jnp.zeros((self.non_block_dim_size, X[self.block_dim].shape[-1])), self.lam_arr, 0)
    ...
    #     return jnp.square(L_inv_R).sum() + self.logdet
...
class Block2x2Kernel(CovType):
    def __init__(self, kf_A, kf_B = None, kf_D_CAB = None, kf_D = None, dim_split = 0, split_loc = 1, split_idx = False):
        self.kf_A = kf_A
