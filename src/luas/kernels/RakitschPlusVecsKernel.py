...
class RakitschPlusVecsKernel(CovType):
    ...
    def _read_K_list(self, K_list, X):
        ...
        return dense_kron, (alpha_mat, low_rank_kernels_dim1), (beta_mat, low_rank_kernels_dim0)
    ...
    def decomp_no_stored_values(
        ...
        dense_kron, *low_rank_terms = self._read_K_list(self.K_list, X)
        ...
        assert low_rank_terms[1-self.fast_dim][0] is None and len(low_rank_terms[1-self.fast_dim][1]) == 0
        ...
        for i in range(self.N_alpha):
            kernel_list[i] = self.Sigma_slow.inv_sqrt_transform(kernel_list[i])
            kernel_list[i] = kernel_list[i].general_transf(self.Q_K_slow_tilde.T)


        ######### Do transforms in fast_dim #########
        # Get Householder transform of all Outer covariances
        stored_values["J_A"], stored_values["U_A"], self.householder_transform = orthonormal_nullspace_gen(alpha_mat, reverse = True)
        ...
        kf_A, _ = kf_A.decompose((X[self.fast_dim][:-self.N_alpha], X[1-self.fast_dim]),
                                 full = False, idx = jnp.arange(self.N_fast - self.N_alpha))
        ...
        V_K = K_fast_tilde.evaluate(X[self.fast_dim], X[self.fast_dim][-self.N_alpha:], full = False,
                                    row_idx = jnp.arange(self.N_fast), col_idx = jnp.arange(self.N_fast-self.N_alpha, self.N_fast))
        ...
        self.V_K_B, V_K_D = V_K[:-self.N_alpha, :], V_K[-self.N_alpha:, :]
        self.V_Sigma_B, V_Sigma_D = V_Sigma[:-self.N_alpha, :], V_Sigma[-self.N_alpha:, :]
        ...
        K_D_dense = jnp.kron(V_Sigma_D, jnp.eye(self.N_slow))
        K_D_dense += jnp.kron(V_K_D, jnp.diag(self.lam_K_tilde))
        ...
        kf_D_CAB = GeneralKernel(*K_alpha, dense_mat = K_D_dense - C_A_inv_B)
        ...
        self.kf_tilde, stored_values["kf_tilde_stored"] = self.kf_tilde.decompose((X[self.fast_dim], X[1-self.fast_dim]), full = True)
